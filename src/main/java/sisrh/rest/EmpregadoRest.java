...
public class EmpregadoRest {
    ...
    @Path("{matricula}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response obterEmpregado(
            @PathParam("matricula") String matricula) throws Exception {
        try{
            Empregado emppregado = Banco.buscarEmpregadoPorMatricula(matricula);
            if(emppregado != null){
                return Response.ok().entity(emppregado).build();
            } else {
                return Response.status(Status.NOT_FOUND).entity("{\"mensagem\" :\"Empregado não encontrado!\"}").build();
            }
        }catch (Exception e) {
            return Response.status(Status.INTERNAL_SERVER_ERROR).entity("{ \"mensagem\" : \"Empregado nao encontrado!\" }").build();
        }
    }
    ...
    public Response excluirEmpregado(@PathParam("matricula") String matricula) throws Exception{
        try{
            if(Banco.buscarEmpregadoPorMatricula(matricula) == null){
                return Response.status(Status.NOT_FOUND).entity("{\"mensagem\" : \"Empregado nao encontrado!\" }").build();
            }
